...
import static deobber.rebuild.nodes.Instruction.getfield;
import static deobber.rebuild.nodes.Instruction.getstatic;
import static deobber.rebuild.nodes.Instruction.invokedynamic;
import static deobber.rebuild.nodes.Instruction.invokeinterface;
import static deobber.rebuild.nodes.Instruction.invokespecial;
import static deobber.rebuild.nodes.Instruction.invokestatic;
import static deobber.rebuild.nodes.Instruction.invokevirtual;
import static deobber.rebuild.nodes.Instruction.multianewarray;
import static deobber.rebuild.nodes.Instruction.new_;
import static deobber.rebuild.nodes.Instruction.newarray;
import static deobber.rebuild.nodes.Instruction.putfield;
import static deobber.rebuild.nodes.Instruction.putstatic;
...
public class DeDuplicate extends Pass implements InstructionVisitor {
	...
	public void execute() {
		...
		Log.log(Log.INFO, "Took %d ms total",
	...
	private int classPass() {
		...
		checkClasses: for (ClassNode node : context.getClasses()) {
			if (!usedClasses.contains(node)) {
				for (Class<?> interf : node.load().getInterfaces()) {
	...
	private int methodPass() {
		...
		// TODO yum, hacky
		if (context.getMappedClasses().get("client").method("init") != null) {
			addMethodUse(null,
					context.getMappedClasses().get("client").method("init"));
		}
		//
	...
	private void addFieldInsn(InsnNode i) {
		...
		FieldNode node = cnode.field(methodName);
		addFieldUse(i, node);
	...
	private void addMethodInsn(InsnNode i) {
		...
		String methodName = ref.getNameType().getName();
		ClassNode cnode = context.getMappedClasses().get(className);
		MethodNode node = cnode.method(methodName);
	...
	private void addClassUse(ClassNode clz) {
		usedClasses.add(clz);
		addMethodUse(null, clz.method("<init>"));
		addMethodUse(null, clz.method("<clinit>"));
	}
	...
	private void addNew(InsnNode i) {
		...
		addMethodUse(null, node.method("<init>"));
	}
